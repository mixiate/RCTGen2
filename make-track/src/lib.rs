...
fn render(
    ...
    for track in &track_desc.tracks {
        ...
        let masks = mask::load_masks(&masks_directory.join(&track.masks).with_extension("json"))?;
        ...
        let track_section_sprite_descs = track_sections
            ...
            .map(|track_section| {
                let views = masks
                    .get(track_section.name)
                    .with_context(|| format!("No mask found for {}.", track_section.name))?;
